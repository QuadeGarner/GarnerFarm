...
export default function EditAnimal() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [animalsRes, ownedRes, animalRes] = await Promise.all([
          fetch("http://localhost:8081/api/animals"),
          fetch("http://localhost:8081/api/animalsOwned"),
          fetch(`http://localhost:8081/api/animalsOwned/${id}`),
        ]);
        ...
        setFormData({
          ...
          streetName: animalData.streetName || "",
    ...
    <>
      <Row className="justify-content-center">
        <Col xs={12} md={10} lg={8} xl={6}>
          <h3 className="mb-4 text-center">Edit Animal</h3>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <Button type="submit" disabled={loading} className="w-100">
              {loading ? <Spinner size="sm" /> : "Update Animal"}
            </Button>
          </Form>
        </Col>
      </Row>
